...
const RegistrationForm = (): JSX.Element => {
  ...
  useEffect(() => {
    if (isFormValidated) {
      API.post("auth/signup", { firstName, lastName, password, email })
        .then((res: AxiosResponse<IRegistrationResponse>) => {
          if (!res.data.success) {
  ...
  return <Container>
    <div className="registration__inner">
      <TextField id="standard-basic" name='firstName' label="First name" required multiline error={alertFirstName}
                 helperText={alertFirstName && 'Not valid First name'}
                 placeholder='John' value={firstName}
                 onChange={(event: ChangeEvent<HTMLTextAreaElement | HTMLInputElement>) => changeFirstName(event.target.value)}/>
      <TextField id="standard-basic" name='lastName' label="Last name" required placeholder='Adams'
                 ...
                 value={lastName}
                 onChange={(event: ChangeEvent<HTMLTextAreaElement | HTMLInputElement>) => changeLastName(event.target.value)}/>
      <TextField id="standard-basic" name='password' label="Password" type="password" required
                 placeholder='somepasswoRd123' value={password} error={alertPassword}
                 helperText={alertPassword && 'Not valid Password'}
                 onChange={(event: ChangeEvent<HTMLTextAreaElement | HTMLInputElement>) => changePassword(event.target.value)}/>
      <TextField id="standard-basic" name='email' label="Email" type="email" required placeholder='youremail@gmail.com'
                 ...
                 value={email}
                 onChange={(event: ChangeEvent<HTMLTextAreaElement | HTMLInputElement>) => changeEmail(event.target.value)}/>
